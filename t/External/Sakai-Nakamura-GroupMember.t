...
ok( $user->add( $test_user, $test_pass. \@test_properties ),
    "Group Member Test: User \"$test_user\" added successfully." );
...
ok( $group->add( $test_group ),
    "Group Member Test: Group \"$test_group\" added successfully." );
ok( $group->check_exists( $test_group ),
    "Group Member Test: Group \"$test_group\" exists." );

ok( $group_member->role_member_add_from_file(),
    "Test role_member_add_from_file function completes successfully." );
ok( $group_member->role_member_delete(),
    "Test role_member_delete function completes successfully." );
ok( $group_member->role_member_exists(),
    "Test role_member_exists function completes successfully." );
ok( $group_member->role_member_view(),
    "Test role_member_view function completes successfully." );

throws_ok { $group_member->role_member_add() } qr{No group name defined to add member to!}, 'Check role_member_add function croaks with no values specified';
ok( $group_member->role_member_add( $test_group, 'manager', $test_user ),
    "Test role_member_add function completes successfully." );
throws_ok { $group_member->role_member_add( '__bad__group__', '__bad__role__', '__bad__user__'); } qr{}, "Test role_member_add function with non-existent group and role.";

# Cleanup Group:
ok( $group->del( $test_group ),
    "Group Member Test: Group \"$test_group\" deleted successfully." );
ok( ! $group->check_exists( $test_group ),
    "Group Member Test: Group \"$test_group\" should no longer exist." );

# Check user deletion:
ok( $user->del( $test_user ),
    "Group Member Test: User \"$test_user\" deleted successfully." );
